...
- Added validated `approvalMode: auto | interactive | agent | block` policy configuration plus the `GUARDME_APPROVAL_MODE` process override. The safe `auto` default preserves TUI approvals while RPC/print/JSON sessions return ordinary bounded fail-closed guidance without emitting an approval UI request; explicit `interactive` retains RPC approval controllers. Opt-in `agent` mode blocks first and same-turn attempts, automatically allows one identical later-turn retry, consumes that approval, and writes a distinct automatic audit record without saving policy.
