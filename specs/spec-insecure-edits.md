...
> Superseded behavior note: `spec-insecure-edits-preserve-path-protections.md` narrows this feature so Insecure edits skips only proposed content/script scanning. Path protections, deny rules, and credential paths still apply to `write` and `edit`.

## Task Description
Add a project-local GuardMe runtime setting that lets users deliberately skip proposed content/script scanning for `write` and `edit` tool calls. The setting should appear in `/guardme` under GENERAL directly below the existing GuardMe toggle and allow generated scripts or other file content to be written to otherwise allowed paths even when that content contains commands that GuardMe would normally block.
...
When `insecureEdits` is `true`, `write` and `edit` tool calls still run path policy evaluation, including deny rules, read-only protection, credential-path protections, and outside-project write requirements. Only proposed content/script scanning is skipped for those two tools. This is intentionally insecure for authoring command-bearing content and should require a confirmation when enabling. Other guarded tools (`bash`, `read`, `grep`, `find`, `ls`) continue to use the existing deny-first policy. Local script execution through `bash` remains inspected and blocked if the script content violates policy.
...
- `src/events/register-guard.ts` - Run `write`/`edit` path policy even when insecure edits are enabled, then skip only proposed content/script scanning.
...
- `test/tool-guard.test.mjs` - Cover content-scan bypass, preserved path protections, and guarded bash execution.
...
### Phase 2: Content-scan bypass
- Add `insecureEdits` to `GuardMeSessionState` during session startup.
- In `evaluateGuardedToolCall`, keep `write`/`edit` path-policy evaluation when GuardMe is enabled and insecure edits are on.
- Skip only proposed content/script scanning for `write`/`edit`; leave all non-edit tools and bash local-script execution behavior unchanged.
...
### 2. Add session state and content-scan bypass
- Add `insecureEdits` to `GuardMeSessionState`.
- Set it from `settings.settings.insecureEdits` in `startGuardMeSession`.
- In `evaluateGuardedToolCall`, after checking GuardMe is enabled, continue path-policy evaluation for `write` and `edit` when `insecureEdits` is true.
- Skip only proposed content/script scanning and ensure `bash` remains guarded, including execution of scripts that were written while insecure edits was on.
...
- Confirm enabling with risk copy that states write/edit skip content scanning while path protections still apply.
...
- When enabled, `write` and `edit` tool calls skip proposed content/script scanning while path protections still apply.
...
## Notes
This feature intentionally weakens write/edit content inspection. The UI and docs should use warning language while making clear that path protections and credential-path blocks still protect file mutations while insecure edits is on.
