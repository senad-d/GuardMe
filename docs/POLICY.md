...
- Tool locations: Homebrew (`/opt/homebrew`, `/usr/local/{bin,lib,opt,Cellar}`), system binaries and libraries (`/bin`, `/usr/bin`, `/usr/lib`, `/usr/share`) and version-manager installs (nvm, Volta, pnpm, asdf, mise) are readable and listable, so `which`, `head -1 $(which tool)` and version checks work; nothing there is writable and credential-like names stay protected.
...
- Functions and traps: `name () { ... }` and `function name { ... }` headers execute nothing; the body is classified as ordinary segments. `trap` with a function defined in the same command or with `-` adds nothing; `trap` with a quoted command string is classified by that string; `trap` with any other bare word stays unclassified.
...
Path protections retain every parsed path/action pair rather than applying the compound command's primary action to all paths. Shell input redirections count as reads and output redirections count as writes, including redirects on wrappers, groups, and loops. Reading a path elsewhere in the command cannot hide a write or delete of that path; read/write redirections (`<>`, `0<>`) must satisfy both actions. Ordinary operands still use their command's classification, including the conservative mixed-operand behavior described below.
