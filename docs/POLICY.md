...
- Project-scoped deletes: `rm` and `rmdir` (including recursive) of concrete paths inside the project are allowed by the default `rm *` and `rmdir *` rules once the path protections pass, so agents can remove their own scratch files. The project root, globs, shell variables, `~`, traversal outside the project, and every other delete-capable command still require an exact allow rule or user approval.
