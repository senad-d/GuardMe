...
The second row toggles **Insecure edits**. Enabling requires confirmation because `write` and `edit` will ignore GuardMe policy rules, path protections, and script-content scanning. This is useful for authoring scripts that contain commands GuardMe should still block at execution time. `bash`, `read`, `grep`, `find`, and `ls` remain guarded, so running a generated local script is still inspected and can be blocked. Disabling Insecure edits restores normal write/edit enforcement without a confirmation.
