...
- `dangerousCommands`: recursive deletion, forced git clean, `find -delete`, and `rsync --delete` patterns, plus file-writing side channels that skip content scanning (`git apply`, `git … core.hooksPath`, `npm pkg set`, `npm set-script`).
