...
export function createBuiltInDefaultPolicy(): GuardMePolicyConfig {
  return {
    ...
    allowCommands: [
      ...
      { pattern: "shellcheck *.sh", reason: "Statically analyze project shell scripts." },
      { pattern: "bash -n *.sh", reason: "Validate Bash syntax without execution." },
      { pattern: "sh -n *.sh", reason: "Validate POSIX shell syntax without execution." },
      { pattern: "shfmt -d *.sh", reason: "Check shell script formatting without modifying files." },
      { pattern: "shfmt -l *.sh", reason: "List shell scripts requiring formatting." },
      { pattern: "checkbashisms *.sh", reason: "Detect Bash-specific syntax." },
      { pattern: "bashate *.sh", reason: "Lint shell scripts." },
