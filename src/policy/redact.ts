...
const HEADER_ASSIGNMENT_PATTERN = /(\b[\w-]+\s*:\s*)("[^"]*"|'[^']*'|[^\s"':]+)/gi;
const SECRET_FLAG_PATTERN = /(^|\s)(--?[A-Z][\w-]*(?:=|\s+))("[^"]*"|'[^']*'|[^\s"']+)/gi;

const SECRET_NAME_MARKERS = ["token", "secret", "password", "pass", "key"] as const;
const SECRET_HEADER_MARKERS = ["api-key", "api_key", "apikey", "token", "secret", "password", "passwd"] as const;
...
export function redactSensitiveText(value: string): string {
  return value
    ...
    .replace(HEADER_ASSIGNMENT_PATTERN, redactSecretHeader)
...
function redactSecretHeader(match: string, prefix: string): string {
  const name = prefix.slice(0, prefix.indexOf(":")).trim().toLowerCase();
  return SECRET_HEADER_MARKERS.some((marker) => name.includes(marker)) ? `${prefix}<redacted>` : match;
}
